...
void
gcal_utils_launch_lingmo_settings (GDBusConnection *connection,
                                  ...
                                  const gchar     *action)
{
  ...
  proxy = create_dbus_proxy (connection, "org.lingmo.Settings", "/org/lingmo/Settings");

  /* Fallback for old Lingmo versions */
  if (!proxy)
    proxy = create_dbus_proxy (connection, "org.lingmo.ControlCenter", "/org/lingmo/ControlCenter");
