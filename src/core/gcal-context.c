...
read_time_format (GcalContext *self)
{
  ...
  ret = g_dbus_proxy_call_sync (self->settings_portal,
                                "Read",
                                g_variant_new ("(ss)", "org.lingmo.desktop.interface", "clock-format"),
                           ...
                           GcalContext *self)
{
  ...
  if (g_strcmp0 (namespace, "org.lingmo.desktop.interface") == 0 &&
...
gcal_context_init (GcalContext *self)
{
  ...
  self->settings = g_settings_new ("org.lingmo.calendar");
